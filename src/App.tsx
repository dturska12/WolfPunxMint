...
import { faTwitter, faDiscord, faTiktok, faYoutube, faTelegram, faMedium, faLinkedin } from '@fortawesome/free-brands-svg-icons';
...
export default function Home() {
  ...
  const theme = (urlParams.get("theme") || themeConst || "light") as
    | "light"
    | "dark";
  const chainParam = urlParams.get("chain") || JSON.stringify({ name: "Ethereum Mainnet" });
  const decodedChainParam = decodeURIComponent(chainParam);
  const chainData = JSON.parse(decodedChainParam);
  const chainName = chainData.name || "Ethereum Mainnet";

  // Remove the word "Mainnet" and convert to lowercase
  const formattedChainName = chainName.replace(/Mainnet/i, "").trim().toLowerCase();
  ...
  return (
    <div className="w-screen min-h-screen">
      <ConnectWallet className="!absolute !right-4 !top-4" theme={darkTheme({
          colors: {
            modalBg: "#000000",
          },
        })}
        ...
        //   title: "Aura ",
        //   titleIcon:
        //     "ipfs://QmQQPQNZ6ZLSs4HsT1J4CwQ9LzTNE8WsoPFLcEvoomuv5p/aura.png",
        //   welcomeScreen: {
        //     img: {
        //       src: "ipfs://QmQQPQNZ6ZLSs4HsT1J4CwQ9LzTNE8WsoPFLcEvoomuv5p/aura.png",
        //       width: 650,
        //       height: 660,
        //     },
        //   },
      ...
      <div className="grid h-screen grid-cols-1 lg:grid-cols-12">
        ...
        <div className="flex items-center justify-center w-full h-full col-span-1 lg:col-span-7">
          <div className="flex flex-col w-full max-w-xl gap-4 p-12 rounded-xl lg:border lg:border-gray-400 lg:dark:border-gray-800">
            ...
            <div className={`flex w-full gap-4 ${themeConst ? 'dark' : 'light'}`}>
              {Object.entries(contractMetadata.data?.social_urls || {}).map(([platform, url]) => {
                let icon = null;
                ...
                if (icon) {
                  return (
                    ...
                    >
                      <FontAwesomeIcon
                        icon={icon as IconProp} // Change the type of icon prop to IconProp
